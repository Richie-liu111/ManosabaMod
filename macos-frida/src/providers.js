...
export function addBackgroundProviders(root, prefix) {
    try {
        ...
        if (!bm) { dbg("[v3] addBackgroundProviders: BackgroundManagerExtended NOT FOUND"); return; }
        var galMi = A.cgm(A.ogc(bm), Memory.allocUtf8String("GetAppearanceLoader"), 1);
        if (!galMi || galMi.isNull()) { dbg("[v3] addBackgroundProviders: GetAppearanceLoader NOT FOUND"); return; }
        ...
        for (var i = 0; i < backIds.length; i++) {
            try {
                ...
                if (!loader || loader.isNull()) { dbg("[v3] 背景 loader '" + backIds[i] + "' 为空"); continue; }
                var lrp = makeLocalResourceProvider(root);
            ...
            } catch (e) { dbg("[v3] 背景 '" + backIds[i] + "' 注入 err: " + e); }
        }
        dbg("[v3] addBackgroundProviders 完成 (" + backIds.join("/") + ")");
    } catch (e) { dbg("[v3] addBackgroundProviders err: " + e); }
}
export function addModLoader(root, prefix) {
    try {
        var sm = findSvc("ScriptManager");
        if (!sm) { dbg("[v3] addModLoader: ScriptManager NOT FOUND"); return; }
        var rl = sm.add(0x28).readPointer();
        if (rl.isNull()) { dbg("[v3] addModLoader: scriptLoader NULL"); return; }

        // 剧本 provider: LRP(MOD_ROOT) + NaniToScriptAssetConverter + ProvisionSource(prefix/Scripts)
        var lrp = makeLocalResourceProvider(root);
        if (lrp.isNull()) { dbg("[v3] addModLoader: LRP 创建失败 (root='" + root + "')"); return; }
    ...
    } catch (e) { dbg("[v3] addModLoader err: " + e); }
}
