...
function chStealResourceGOClass() {
    try {
        ...
        if (out) {
            ...
            info("[Choice] 偷到 Resource<GameObject> klass=" + A.cgn(out).readCString() + " (from " + (chData.resGOClass ? "loaded cache" : "cache") + ")");
        }
        return out;
    }
    catch (e) {
...
function chRegisterProvider() {
    try {
        ...
        if (addMi && !addMi.isNull()) {
            ...
            if (ar.ok)
                info("[Choice] providersMap.Add('" + chData.providerKey + "', vrp) OK");
            else {
        ...
        try {
            ...
            if (gpMi && !gpMi.isNull()) {
                ...
                if (gret && !gret.isNull() && gret.equals(chData.vrp))
                    info("[Choice] GetProvider('" + chData.providerKey + "') ✓ 就是我们的 vrp");
                else {
                            ...
                            try {
                                ...
                                if (mnm === "Load" && tag === "RL-P") {
                                    try {
                                        ...
                                        dbg("[Choice] ProvisionSources 诊断 err: " + e3);
                                    }
                                }
                                ...
                                if (mnm === "Load" && tag === "RL-P" && path.indexOf("MyMod") >= 0) {
                                    try {
                                        ...
                                        var ga2 = null;
                                        try {
                                            ga2 = Process.getModuleByName("GameAssembly_arm64.dylib");
                                        }
                                        catch (e) { }
                                        var b2 = ga2 ? ga2.base : ptr(0);
                                        var names = bt.map(function (ad) {
                                            try {
                                                if (b2 && ad.compare(b2) >= 0 && ad.compare(b2.add(0x7000000)) < 0)
                                                    return "0x" + ad.sub(b2).toString(16);
                    ...
                    onEnter: function (a) {
                        try {
                            ...
                            if (id && id.indexOf("Trial") !== 0)
                                info("[Choice] 游戏构造 UIChoiceHandler '" + id + "' (Initialize)");
                        }
                        catch (e) { }
                    }
                });
                    ...
                    Interceptor.attach(goaMi.readPointer(), {
                        onEnter: function (a) {
                            try {
                                var id = readStr(a[1]);
                                if (id && id.indexOf("Trial") !== 0)
                                    info("[Choice] 游戏 GetOrAddActor('" + id + "')");
                            }
                            catch (e) { }
                        }
                    });
