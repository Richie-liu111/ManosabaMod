...
架构现状：`src/` 多文件 ES modules 工程，`frida-compile` 打包单 bundle 注入（`dist/manosabamod.js`）。机制日志走 `MOD_DEBUG` 开关（默认关），游戏侧 `Unity.LogError` 全量抓取为最高优先级信号（[ARCHIVE 教训 2/3](../../../ARCHIVE_2026-08-03.md)）。

## Windows vs macOS 功能对照

| Windows 模块 (ManosabaMod-2.0.0/ManosabaLoader/) | 功能 | macOS 状态 |
|------|------|------|
...
1. **mod 自定义 ChoiceHandler（魔女裁判环节的 mod 面板）** — ❌ 未闭环
   - 场景：mod 剧本用 `@choice handler:"<modId>"`（样例 [试试写一个魔女裁判.md](../../manosaba_docs/试试写一个魔女裁判.md#L143)，原版审判是 `handler:"Trial"`）指定 mod 的克隆面板+立绘。
...
2. **CutIn** — ⚠️ 已装未实测（SetVariableValue 改写 objectionCutInSpawnPath + SetSpawnParameters 换 sprite；仅 sprite 替换无 shader）
...
5. **菜单翻页** — ✅ 2026-08-03 已回迁（perPage=4，`ChoiceList_<页>` 方案，镜像 Windows AddModStartMenu），随全量回归验证。

## 参考

- 排查经验/根因链/教训：`~/manosaba decompile/ARCHIVE_2026-08-03.md`
- Windows 参考实现：`~/manosaba decompile/ManosabaMod-2.0.0/ManosabaLoader/`
- 魔女裁判 mod 写法：`~/manosaba decompile/manosaba_docs/试试写一个魔女裁判.md`
- 16h @choice 排查现场：git 分支 `save/16h-choice`
